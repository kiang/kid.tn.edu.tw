...
$year = date('Y');
$yearPath = __DIR__ . '/data/lot2/' . $year;
if (!file_exists($yearPath)) {
    mkdir($yearPath, 0777, true);
}

$client = new Client();
$crawler = $client->request('GET', 'https://kid.tn.edu.tw/KidsAll/Public/Lot2QryP.aspx');
...
foreach ($areas as $area) {
    $yearLotPath = __DIR__ . '/raw/lot2p/' . $year;
...
$crawler = $client->request('GET', 'https://kid.tn.edu.tw/KidsAll/Public/Lot2QryN.aspx');
$form = $crawler->filter('form')->form();
$domdocument = new \DOMDocument;
$ff = $domdocument->createElement('input');
$ff->setAttribute('name', 'ctl00$MainContent$btnQry');
...
foreach ($areas as $area) {
    $yearLotPath = __DIR__ . '/raw/lot2n/' . $year;
    if (!file_exists($yearLotPath)) {
        mkdir($yearLotPath, 0777, true);
    }
    $crawler = $client->submit($form, array('ctl00$MainContent$ddlArea' => $area));
